  ...
  <div class="lx_container">
    <div class="lx_main">
      ...
      <block-border>
        ...
        <el-form ref="projform" slot="container" label-width="160px" :rules="rules" :model="form" id="hocRequest"
          ...
          <el-form-item label="合作协议书" class="is-required" id="fileItem" :class="{fileStyle : fileStyleFlag}">
          ...
          <el-form-item label="签字后的合作协议书" class="lx_level_main">
            ...
            <file-upload :attaObj="fileObj6" @files="_getFileID6" :isDisable="false" :isTip="false"
                         :attaList="fileList6"
                         key="two"></file-upload>
          </el-form-item>
      ...
      <block-border>
        ...
        <el-form slot="container" :model="form" label-width="140px" :rules="rules" class="lx_form_baseinfo"
          ...
          <!--本次合作等级-->
          <coop-level :level-list="leavelList" :level-item="form.editlevel"
        ...
        <el-form v-if="commTmpFlag" :model="commForm" ref="tmpbase" label-width="130px" slot="container"
          ...
          <el-form-item label="按固定佣金" v-show="commFixationTmpFlag">
            <div class="lx_flex_broke lx_pdb_10" v-for="(item,index) in fixaList" :model="fixaList">
              ...
              <el-input v-model="item.amount" placeholder="输入佣金金额" maxlength="10"
          ...
          <el-form-item label="按费用百分比" v-show="commPercentTmpFlag" :model="percentList">
            <div class="lx_flex_broke lx_pdb_10" v-for="(item,index) in percentList">
              ...
              <el-input v-model="item.percent" placeholder="费用占比" maxlength="10"
              ...
              <div class="lx_grads_body">
                <div class="lx_grads_row" v-for="(item,index) in gradList" :key="index">
                  <div class="lx_grads_nums">
                    ...
                    &nbsp;&nbsp;≤&nbsp;&nbsp;
                    <el-input size="small" class="lx_width_70" placeholder="请输入" v-model="item.maxNum" maxlength="10"
                  ...
                  <div class="lx_grads_charge">
                    ...
                    <div class="lx_grads_item">
                      <el-input v-if="item.zanbiFlag" v-model="item.percent" placeholder="费用占比" size="small"
                                class="lx_width_110" maxlength="10"
                                :class='item.percentFlag ? {fixaAmoutBorderColor : true} : ""'></el-input>
          ...
          <el-form-item label="额外奖励">
            <el-input
              :autosize="{ minRows: 4, maxRows: 6}"
              type="textarea"
              v-model="commForm.extraBonus" maxlength="500" placeholder="输入额外奖励条款，此部分金额不支持自动计算"></el-input>
        ...
        <el-form v-else :model="commForm" ref="tamcont" :rules="rules" label-width="130px" slot="container"
          ...
          <el-form-item label="备注" prop="memo" class="memoClass">
            <el-input :autosize="{ minRows: 4, maxRows: 6}"
                      type="textarea"
                      maxlength="500"
                      v-model="commForm.desc"
...
<style lang="scss" rel="stylesheet/scss">
  ...
  .el-form-item.memoClass:not(.is-no-asterisk) > .el-form-item__label:before {
    content: '';
  }
...
import { ChangeCommMixin, ApplyFileMixin, FormValit, ParentValMixin } from '@/utils/mixin'
...
export default {
  mixins: [ChangeCommMixin, ApplyFileMixin, FormValit, ParentValMixin],
  data() {
    return {
      form: {
        applyUser: null, // 申报人
    ...
    _initApply(id) {
      ...
      getApplyDetail({
        ...
        if (res.status === ERR_OK) {
          ...
          this.isUpload = res.data.isUpload === null ? false : res.data.isUpload
          ...
          getLevelDetail({
            ...
            if (r.status === ERR_OK) {
              console.log('院校等级')
              this.form.editlevel = r.data === null ? null : r.data.leavel
          ...
          getDirectCommDetail({
            ...
            if (res.status === ERR_OK) {
              ...
              let comms = res.data.comm === null ? null : res.data.comm
              // this.commtmp = res.data.comm.type === null ? '' : res.data.comm.type.code
              this.commtmp = comms === null ? 'STANDARD' : res.data.comm.type.code
              this.commForm.memo = comms === null ? null : res.data.comm.memo
              this.commForm.desc = comms === null ? null : res.data.comm.desc
              if (comms === null) {
                return
              } else {
...
//              })
  ...
  computed: {
    ...
    isRenew() {
      if (this.schoolInfo.tagTypeCode === 'RENEW') {
        return true
      } else {
        return false
      }
