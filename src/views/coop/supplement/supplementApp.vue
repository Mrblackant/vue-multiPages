<template>
  <div class="lx_container">
    <div class='lx_main'>
      <sub-header icon='' title='合作项目补录'></sub-header>
      <block-border>
        ...
        <el-form slot="container" :model='form' label-width='120px'>
          ...
          <el-row>
            ...
            <el-col :span='8'>
              <el-form-item label='是否补录完成'>
                <el-select v-model='isFinshed' placeholder='请选择' class='width100'>
                  <el-option v-for="(item,index) in finshedList" :label='item.name' :value='item.key'
                             :key="index"></el-option>
                </el-select>
              </el-form-item>
            </el-col>
...
import {getApplyList} from '@/api/coopManage'
...
export default {
  ...
  methods: {
    ...
    formatterReplenish(row) {
      switch (row.replenish) {
        case false:
          return '是'
        break;
        default:
...
//        })
    ...
    isEdit(row) {
      ...
      if (row.status.code === 'DRAFT' || row.status.code === 'REJECT' || row.status.code === 'REVOKE') {
    ...
    goEdit(row) {
      if(row.approveConType){
        ...
        this.$alert('该项目已提交' + type + '申请正在审批中，当前不容许补录！', '错误提示', {
      ...
      if (row.applyType.code === global_.DIRECTCOL) {
        //window.location.href = 'suppdir.html?id=' + row.id
        window.open('suppdirect.html?id=' + row.id)
//          this.$router.push({
//            name: 'SupplementEditDri',
//            query: {
//              id: row.id
//            }
//          })
      ...
      if (row.applyType.code === global_.ORGCOL) {
        // window.location.href = 'supporg.html?id=' + row.id
        window.open('supporg.html?id=' + row.id)
//          this.$router.push({
//            name: 'SupplementEditOrg',
//            query: {
//              id: row.id
//            }
//          })
