  ...
  <div class="lx_container">
    <div class="lx_main">
          ...
          <el-table :data="selectedSchoolData"
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                {{ levalTemp[scope.row.originalLeavel] }}
              </template>
            </el-table-column>
      ...
      <block-border>
        ...
        <el-form ref="projform" slot="container" :rules="rules" label-width="160px" :model="form" id="hocRequest"
          ...
          <el-form-item label="签字后的合作协议书" class="lx_level_main">
            ...
            <file-upload :attaObj="fileObj6" @files="_getFileID6" :isDisable="false" :isTip="false" :attaList="fileList6"
                         key="two"></file-upload>
          </el-form-item>
      ...
      <block-border>
        ...
        <div slot="container">
          <el-form :model="form" label-width="150px" class="lx_form_label_16" id="comInfo">
            <!--佣金联系人-->
            <comm-link :payList="payTypeList" :comm-link="form.commContacts" @commform="_getCommLink" ref="commlink"></comm-link>
          </el-form>
          <!--非标准模板-->
            ...
            <el-form label-width="150px" class="lx_form_label_16" :disabled="true">
              <el-form-item label="按固定佣金" v-if="item.hasOwnProperty('mouldType') && item.mouldType==='FIXED'">
                <div class="lx_flex_broke lx_pdb_10" v-for="(n,index) in item.commDetailRequestList">
                  ...
                  <el-input v-model="n.amount" placeholder="输入佣金金额" class="lx_flex_item lx_flex_2"
              ...
              <el-form-item label="按费用百分比" v-if="item.hasOwnProperty('mouldType') && item.mouldType==='COST_PERCENT'">
                <div class="lx_flex_broke lx_pdb_10" v-for="(n,index) in item.commDetailRequestList">
                  ...
                  <el-input v-model="n.chargeItem" placeholder="输入费用名称" class="lx_flex_item lx_flex_2"
                            maxlength="100"></el-input>
                  <el-input v-model="n.percent" placeholder="费用占比" maxlength="10"
                ...
                <div class="lx_grads">
                  ...
                  <div class="lx_grads_body">
                    <div class="lx_grads_row" v-for="(n,index) in item.commDetailRequestList" :key="index">
                      <div class="lx_grads_nums">
                        ...
                        &nbsp;&nbsp;≤&nbsp;&nbsp;
                        <el-input size="mini" class="lx_width_70" placeholder="请输入" v-model="n.maxNum"
                                  maxlength="10"></el-input>
                      </div>
                  ...
                  <span class="lx_grads_span">
                    ...
                    <el-input v-if="n.mouldDetailType!==null&&n.mouldDetailType==='COST_PERCENT'" v-model="n.chargeItem"
                              ...
                              class="lx_width_110"
                              size="mini"></el-input>
                    <el-input v-else v-model="n.amount" placeholder="费用金额" class="lx_width_110" maxlength="10"
                              size="mini"></el-input>
                  </span>
                        <span class="lx_grads_span">
                    <el-input v-if="n.mouldDetailType!==null&&n.mouldDetailType==='COST_PERCENT'" v-model="n.percent"
                              placeholder="费用占比" maxlength="10"
                              size="mini"
                              class="lx_width_110"></el-input>
              ...
              <el-form-item v-if="item.hasOwnProperty('extraBonus')&&item.extraBonus!==null" label="额外奖励">
                <el-input
                  :autosize="{ minRows: 4, maxRows: 6}"
                  type="textarea"
                  v-model="item.extraBonus" placeholder="输入额外奖励条款，此部分金额不支持自动计算" maxlength="500"></el-input>
              </el-form-item>
      ...
      <div class="lx_btn_center">
        ...
        <el-checkbox border v-model="isFinish">已补全信息</el-checkbox>
        <div style="color: red">选中该选项，保存后，当前合作信息则视为已补全，将不在补录页面展示。只能在新系统中查询。</div>
      </div>
      <!--设置合作等级开始-->
      <el-dialog
            ...
            <el-row :gutter="40">
              ...
              <el-col :span="10">
                <el-form-item label="院校名称">
                  <el-input v-model="form.schoolName" placeholder="请输入" maxlength="200"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="6">
...
import {CountryMixin, ApplyFileMixin, FormValit, GetCommSchoolListMxin} from '@/utils/mixin'
...
export default {
  mixins: [CountryMixin, FormValit, ApplyFileMixin, GetCommSchoolListMxin],
  data() {
    return {
  ...
  methods: {
    ...
    submitApply(type) {
      ...
      let list = this.coopApplyCommList1.slice(0)
      let params = {
        commContacts: this.commLink.commContacts, // 佣金联系人
        contactsList: this.schoolInfoList, // 机构联系人
        address: this.linkform.colAddress,
        addressCn: this.linkform.colAddress,
        website: this.linkform.colWebsite,
        attachments: this._formatAtta(), // 附件
        colOrgList: this._formatterOrgList(), // 院校列表
        coopApplyCommList: this._formatSubmitComm(list), // 模板列表
        projectName: this.form.projName, // 机构名称
        startTime: this._parTime(this.form.startdate), // 合同开始日期
        endTime: this._parTime(this.form.enddate), // 合同结束日期
        signInfo: this.form.pastyearInfo, // 去年送生情况
        memo: this.form.memo, // 项目基本情况说明
        ...
        conType: 'NEW', // 合同类型
        coopOrgId: this.orgId,
        payType: this.commLink.commpayType, // 支付方式
        id: this.queryID,
        operatorType: 'TYPE1',
        replenish: !this.isFinish
      }
    ...
    countryData: {
      handler(newVal, oldVal) {
        if (newVal) {
          // console.log(newVal)
          let that = this
          if (that.listOfOrgCountry && that.listOfOrgCountry.length > 0) {
            newVal.map(i => {
              // 此处产品要求，当机构有多个国家时，默认选中第一个
              if (i.code === that.listOfOrgCountry[0].code) {
