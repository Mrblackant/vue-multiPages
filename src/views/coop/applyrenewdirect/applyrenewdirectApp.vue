    ...
    <div class="lx_main">
      ...
      <block-border>
        ...
        <div slot="container" class="lx_container">
          <el-row class="lx_row_txt">
            <el-col :span="8"><span>申请类型：{{schoolInfo.applyType}}
      ...
      <block-border>
        ...
        <div slot="container" class="lx_container clearfix">
          <el-form :model="form" label-width="120px" :rules="rules" class="lx_form_baseinfo" id="basicMsg"
                   ref="baseForm">
            <el-row class="lx_row_txt">
              <el-col :span="8"><span class="lx_row_keys">院校中文名称：</span><span
                class="lx_row_values">{{schoolInfo.nameCn}}</span></el-col>
              <el-col :span="16"><span class="lx_row_keys">院校外文名称：</span><span
                class="lx_row_values">{{schoolInfo.nameEn}}</span></el-col>
            </el-row>
            <el-row class="lx_row_txt">
              <el-col :span="8"><span class="lx_row_keys">院校性质：</span><span>{{property}}</span></el-col>
              <el-col :span="8"><span class="lx_row_keys">院校类别：</span><span>{{colType}}</span></el-col>
            </el-row>
            <el-row class="lx_row_txt">
              <el-col>
                <span class="lx_row_keys">合作项目名称：{{form.projName}}</span>
              </el-col>
            </el-row>
            <el-row>
              <el-col :span="8">
              ...
              <el-col :span="16">
                <el-form-item label="合同结束日期" prop="enddate">
                  ...
                  <el-checkbox v-if="!isSupp()" @change="changeForever" v-model="form.isForever" border>永久</el-checkbox>
            ...
            <el-col>
              <el-form-item label="去年送生情况" prop="pastyearInfo">
                <el-input v-model="form.pastyearInfo" maxlength="250" placeholder="请输入" class="inline"></el-input>
              </el-form-item>
            </el-col>
          </el-form>
        </div>
      ...
      <block-border>
        ...
        <el-form slot="container" :model="form" label-width="140px" :rules="rules" class="lx_form_baseinfo"
          ...
          <!--本次合作等级-->
          <coop-level :level-list="leavelList" :isRenews="true" :renewString="form.originalLevel"
                      :level-table="levelTableList"
                      @level="_getLevel"></coop-level>
          <!--佣金模板和类型-->
        ...
        <el-form v-if="commTmpFlag" :model="commForm" ref="tmpbase" label-width="130px" slot="container"
          ...
          <el-form-item label="按固定佣金" v-show="commFixationTmpFlag" :model="fixaList">
            <div class="lx_flex_broke lx_pdb_10" v-for="(item,index) in fixaList" :model="fixaList">
              ...
              <el-input v-model="item.amount" placeholder="输入佣金金额" class="lx_flex_item lx_flex_2"
                        :class='item.amountFlag ? {fixaAmoutBorderColor : true} : ""' maxlength="10"
                        type="text"></el-input>
              ...
              <div class="lx_grads_item" v-if="index>=1">
          ...
          <el-form-item label="按费用百分比" v-show="commPercentTmpFlag" :model="percentList">
            <div class="lx_flex_broke lx_pdb_10" v-for="(item,index) in percentList">
              ...
              <el-input v-model="item.percent" placeholder="费用占比" maxlength="10" type="text"
              ...
              <div class="lx_grads_body">
                <div class="lx_grads_row" v-for="(item,index) in gradList" :key="index">
                  ...
                  <div class="lx_grads_charge">
                    ...
                    <div class="lx_grads_item">
                      <comm-category title="请选择留学类别" :width="typeWidth" :parentVal="parentVal(item.type)"
                                     :dataList="schoolTypes" :key="index"
                                     :values.sync="item.type" button-size="small"
                                     :class='item.typeFlag ? {fixaTypeBorderColor : true} : ""'
                                     category-cla="lx_category"></comm-category>
                    </div>
                    <div class="lx_grads_item">
                    ...
                    <div class="lx_grads_item">
                      <el-input v-if="item.zanbiFlag" v-model="item.percent" placeholder="费用占比" size="mini"
                    ...
                    <div class="lx_grads_item" v-if="index>=1&&index===gradList.length-1">
        ...
        <!--标准模板结束-->
        <!--非标准模板开始-->
        <el-form v-else :model="commForm" ref="tamcont" label-width="130px" :rules="rules" slot="container"
                 class="lx_form_tmp">
...
import { ChangeCommMixin, ApplyFileMixin, EditFormValit, ParentValMixin } from '@/utils/mixin'
import {
  getCoopConDetail,
...
export default {
  mixins: [ChangeCommMixin, ApplyFileMixin, EditFormValit, ParentValMixin],
  data() {
    return {
      form: {
  ...
  mounted() {
    ...
    this.originalConId = req.id
    this.conType = req.conType
    window.document.getElementsByTagName('title')[0].innerText = this._getConType() + '--院校直接合作项目'
    if (!this.queryID) {
  ...
  methods: {
    ...
    _initApply(id) {
      ...
      getCoopConDetail({
        ...
        if (res.status === ERR_OK) {
          console.log('申请详情成功')
          this.schoolInfo.applyType = res.data.applyType.name
          this.schoolInfo.tagType = res.data.conType.name
          // this.schoolInfo.countryName = res.data.conType
      ...
      getConDirectLevelDetail({
        ...
        if (r.status === ERR_OK) {
          console.log('院校等级')
          // this.form.level = r.data.leavel
          this.form.originalLevel = r.data.leavel
          // 院校佣金
          getConDirectCommDetail({
            conId: id
          }).then(res => {
    ...
    _initParam(type) {
      ...
      const params = {
        id: this.updateID,
        attachments: this._formatAtta(), // 附件
        operatorType: type,
        ...
        leavel: this.form.renewLevel, // 合作等级
        colId: this.colId, // 院校ID
        isUpload: this.isUpload, // 签字后合作协议书
        originalLeavel: this.form.originalLevel,
        originalConId: this.queryID
      }
      // 非标准模板
      const nostand = {
        coopApplyComm: {
    ...
    submitProject(type) {
      ...
      //         window.location.href = 'applydetail.html?id=' + res.data + '&type=school&derive=false&edit=false'
      //     }
      //   } else {
      ...
      this.$refs.baseForm.validate((valid) => {
        if (valid) {
          if (that._vali()) {
            ...
            if (that.commLink.commpayType === undefined || that.commLink.commpayType === '') {
